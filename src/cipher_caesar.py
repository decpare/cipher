...
def encrypt(input_text, shift, abc, second_abc, formatting, save_case):
    if shift == "" or shift == 0:
        return input_text
    if save_case is False:
        input_text = list(str(input_text).lower())
    ...
    for s in input_text:
        ...
        if s in abc:
            ...
            output_text += abc[
                (abc.index(s.lower()) + shift) % abc_len].upper()
        elif s in second_abc:
            output_text += second_abc[(second_abc.index(s) + shift) %
                                      second_abc_len]
        elif s_lower in second_abc:
            output_text += second_abc[(second_abc.index(s.lower()) + shift) %
                                      second_abc_len].upper()
        else:
...
if __name__ == "__main__":
    ...
    if mode != "e" and mode != "d":
        print("Uncorrect command!"
              "Please close the program (Ctrl+Z) and restart it...")
    ...
    if mode == "e":
        output_text.write(
            encrypt(input_text, shift, alphabet.rus_alphabet,
                    alphabet.eng_alphabet, True, save_case))
    elif mode == "d":
        output_text.write(
            decipher(input_text, shift, alphabet.rus_alphabet,
                     alphabet.eng_alphabet, True, save_case))
    else:
